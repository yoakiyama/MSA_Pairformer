...
from MSAPairformer.outer_product import DifferentialOuterProductMean
...
experiment_name = "train_presoftmax_diff_attn"
...
def main():
    ...
    params = [
        {'params': [p for n, p in model.named_parameters() if any(param_name in n for param_name in higher_lr_param_l)], 'weight_decay': weight_decay},  # Higher learning rate for attention params
        {'params': [p for n, p in model.named_parameters() if not any(param_name in n for param_name in higher_lr_param_l)], 'weight_decay': weight_decay_base}  # Default learning rate for other params
    ]
    ...
    scheduler = lr_scheduler.OneCycleLR(
        ...
        final_div_factor=1e5,#lr/lr_min,
        anneal_strategy='cos',
    )
