...
class Attention(Module):
    ...
    def forward(
        self,
        seq: Float['b i d'], # b*n, n, d
        mask: Bool['b n']| None = None,
        context: Float['b j d'] | None = None,
        windowed_mask: Bool['b nw w (w*2)'] | None = None,
        attn_bias: Float['... i j'] | Float['... nw w (w*2)'] | None = None # b*n, h, n, n

    ) -> Float['b i d']:

        q = self.to_q(seq) # b*n, n, d

        context_seq = default(context, seq)
        k, v = self.to_kv(context_seq).chunk(2, dim = -1) # b*n, n, d

        q, k, v = tuple(self.split_heads(t) for t in (q, k, v)) # b*n, h, n, d
        ...
        out = self.merge_heads(out) # b*n, n, d
...
class Attend(Module):
    ...
    def local_attn(
        self,
        q: Float['b h n d'], # b*n, h, n, d
        k: Float['b h n d'], # b*n, h, n, d
        v: Float['b h n d'], # b*n, h, n, d
        mask: Bool['b n'] | None = None,
        windowed_mask: Bool['b nw w (w*2)'] | None = None,
        attn_bias: Float['... n n'] | Float['... nw w (w*2)'] | None = None, # b*n, h, n, n
