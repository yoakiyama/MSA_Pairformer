...
from torch.nn import Module, ModuleList, Linear, Sequential, ModuleDict
...
from .core import LinearNoBias, PreRMSNorm, Transition, exists
from .outer_product import OuterProduct
from .regression import LMHead, LogisticRegressionContactHead
from .pairwise_operations import MSAPairWeightedAveraging, PairwiseBlock
from .positional_encoding import RelativePositionEncoding, BasicRelativePositionEncoding
    ...
    def __init__(
        ...
        *,
        ...
        for layer_idx in range(depth):
            ...
            msa_pre_ln = partial(PreRMSNorm, dim = dim_msa)
            ...
            # Skip if last layer since the final pair rep would not be backpropped through
...
class MSAPairformer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        # MSA module
        self.msa_module = CustomMSAModule(
            dim_msa = dim_msa,
            dim_pairwise = dim_pairwise,
            **msa_module_kwargs
        )
    ...
    def forward(
        self,
        msa: Float['b s n d'], 
        additional_molecule_feats: Float['b n a'] | None = None,
        ...
        if self.use_basic_relative_position_encoding:
            b, _, n, _ = msa.shape
            token_indices = torch.arange(n, device=msa.device).unsqueeze(0).expand(b, n)
            pairwise_repr = self.relative_position_encoding(token_indices=token_indices)
        else:
...
class MSAPairformerContactModel(Module):
    """
    ...
    def __init__(
        ...
        self.pretrained_model = MSAPairformer(
            **msa_model_kwargs
        )
