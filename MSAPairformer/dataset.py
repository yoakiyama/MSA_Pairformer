...
class MSA:
    def __init__(
        ...
        subset_depth = min(self.max_tokens // sequence_length, self.max_seqs)
        if random_depth:
            subset_depth = np.random.randint(1, subset_depth)
        self.subset_depth = subset_depth
        self.select_diverse_msa, self.select_diverse_indices = self.select_diverse(msa_a=self.random_crop, num_seqs=subset_depth, method=diverse_select_method, hhfilter_kwargs=hhfilter_kwargs)
...
def msa_mlm(
    ...
    if query_only:
        ...
        non_pad_indices = non_pad_indices[flat_msas[non_pad_indices] != pad_tok] # This model's alphabet
    else:
...
def prepare_additional_molecule_feats(msa_input):
    ...
    return additional_molecule_feats
