...
from MSAPairformer.core import to_pairwise_mask, LinearNoBias, SwiGLU, exists
from MSAPairformer.custom_typing import (
    Float,
    Bool
)
import sys
sys.path.append("/home/ubuntu/openfold/openfold/utils/")
...
class OuterProductMean(Module):
    def __init__(
        ...
        if self.seq_attn:
            ...
            self.q_norm = RMSNorm(dim_qk)
            self.k_norm = RMSNorm(dim_qk)
        self.return_seq_weights = return_seq_weights
        self.return_attn_logits = return_attn_logits
    def _opm(self, a, b):
    ...
    def forward(
        ...
        if not self.return_seq_weights:
            del seq_weights
            seq_weights = None
        if not self.return_attn_logits:
            del attn_logits
            attn_logits = None
        return outer, seq_weights, attn_logits

class DifferentialOuterProductMean(Module):
    def __init__(
        self,
        dim_hidden = 32,
        dim_msa = 64,
        ...
        self.eps = eps
        self.chunk_size = chunk_size
        # Initialize RMSNorm to normalize input MSA representation
        self.norm = RMSNorm(dim_msa)
        ...
        self.return_seq_weights = return_seq_weights
        assert not return_attn_logits, "DifferentialOuterProductMean does not return attention logits"
    ...
    def forward(
        ...
        # Default to full mask if not provided
        if not exists(full_mask):
            full_mask = msa.new_ones(msa.shape[:-1]) # [b, s, n]
        # Normalize MSA representation
        norm_msa = self.norm(msa)
        # Unsupervised sequence weight learning
        if self.seq_attn:
            # Method 1: Average the attention weights over all positions for each sequence
            ...
            # Average over all positions for each sequence (Apply mask to seq weights before averaging)
            norm_factor = (full_mask.sum(dim=-1) + self.eps).unsqueeze(1).expand(-1, 2, -1) # [b, 2, n]
            ...
            # Apply attention mask and compute softmax attention weights
            seq_weights = seq_weights.masked_fill(~msa_mask.unsqueeze(1).expand(-1, 2, -1), -1e9)
            seq_weights = seq_weights.softmax(dim=-1)  # Softmax over sequence dim [b 2 s]
            # Compute lambda (potentially where we run into numerical instability)
            ...
            seq_weights = seq_weights[:, 0, :] - (lambda_full * seq_weights[:, 1, :])
        ...
        b = b.transpose(-2, -3) # [b n s c]
        # Scale a and b by the square root of the sequence weights
        # Possible that attention is negative. Save sign and multiply one side by sign (subtracts out noise)
        attn_sign = torch.sign(seq_weights)
        scaled_seq_weights = ((seq_weights * attn_sign) + self.eps).sqrt()
        a = torch.einsum("...s,...nsc->...nsc", scaled_seq_weights * attn_sign, a) # [b n s c]
        b = torch.einsum("...s,...nsc->...nsc", scaled_seq_weights, b) # [b n s c]
        if self.chunk_size is not None:
            outer = self._chunk(a, b, self.chunk_size)
        else:
...
class NonNegativeDifferentialOuterProductMean(Module):
    def __init__(
        self,
        dim_hidden = 32,
        dim_msa = 64,
    ...
    def forward(
        ...
        if self.seq_attn:
            ...
            # Compute differential attention
            seq_weights = seq_weights[:, 0, :] - (lambda_full * seq_weights[:, 1, :])
            # Make sure values are non-negative and renormalize
            seq_weights = seq_weights.relu()
            seq_weights = seq_weights / (seq_weights.sum(dim=-1, keepdim=True) + self.eps)
            del lambda_1, lambda_2, lambda_full, q, k
        else:
            # Default to uniform sequence weights
            if not exists(seq_weights):
...
class OuterProduct(Module):
    def __init__(
        ...
        dim_pairwise: int,
        dim_opm_hidden: int,
        outer_product_flavor: Literal["vanilla", "vanilla_attention", "differential_attention", "non_negative_differential_attention", "presoftmax_differential_attention"],
        seq_attn: bool = False,
        dim_qk: Optional[int] = None,
        chunk_size: Optional[int] = None,
        return_seq_weights: Optional[bool] = False,
        return_attn_logits: Optional[bool] = False,
        lambda_init: Optional[float] = None,
        eps: float = 1e-32,
        nSeqs: Optional[int] = None,
        nResidues: Optional[int] = None
    ):
        super().__init__()
        assert outer_product_flavor in ["vanilla", "vanilla_attention", "differential_attention", "non_negative_differential_attention", "presoftmax_differential_attention"]
        ...
        elif outer_product_flavor == "differential_attention":
            assert (dim_qk is not None) and (lambda_init is not None), "dim_qk and lambda_init must be provided for differential_attention"
            self.opm = DifferentialOuterProductMean(
                dim_hidden = dim_opm_hidden,
                dim_msa = dim_msa,
                dim_pairwise = dim_pairwise,
                chunk_size = chunk_size,
        ...
        elif outer_product_flavor == "non_negative_differential_attention":
            self.opm = NonNegativeDifferentialOuterProductMean(
                dim_hidden = dim_opm_hidden,
                dim_msa = dim_msa,
                dim_pairwise = dim_pairwise,
                chunk_size = chunk_size,
